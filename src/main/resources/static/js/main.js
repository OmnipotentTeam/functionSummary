...
function initMenu(){
	 $.ajax({  
	     url:httpUrl+"permissions/current",
	     type:"get",  
	     async:false,
...
function showUnreadNotice(){
	$.ajax({
		type : 'get',
		url : httpUrl+'notices/count-unread',
		success : function(data) {
    ...
    element.on('nav(demo)', function(elem){
	  ...
	  //触发事件  
	   ...
	   function resize(){  
	       ...
	       $content.height($(this).height() - 186);
	       $content.find('iframe').each(function() {
	           $(this).height($content.height());
	           $(this).css("border-radius","8px");
	           $(".layui-tab-item ").height($content.height());
	       });
	   }  
	   ...
	   $('.menu_change').on('click', function() {
	       ...
	       if(sideWidth === 200) {  
			   ...
			   $("#biaozhi").attr("class","fa fa-chevron-left");
			   $("#biaozhi").prop("class","fa fa-chevron-right");
			   $(".menu_change").attr("static.img.background.title","显示菜单");
			   ...
			   $("#biaozhi").attr("class","fa fa-chevron-right");
			   $("#biaozhi").prop("class","fa fa-chevron-left");
			   $(".menu_change").attr("static.img.background.title","隐藏菜单");
	    ...
	    //隐藏首页的关闭按钮
		$("#welcome>i")[$("#welcome>i").length-1].style.display = "none";
});