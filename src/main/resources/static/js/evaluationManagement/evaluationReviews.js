...
function TableInit() {
    ...
    oTableInit.Init = function () {
        $('#evaluationQuery').bootstrapTable({
            url: httpUrl+"/mark/selectMark",         //请求后台的URL（*）
            method: 'POST',                      //请求方式（*）
...
function addFunctionAlty(value, row, index) {
    ...
    btnText += "<input type=\"button\" class=\"layui-btn layui-btn-xs btn-see\" onclick=\"evaluation(" + "'" +  row.markId + "'" + "," + "'" +  row.studentId + "'" + "," + "'" +  row.gradeEvaluationId + "'" + ")\" value=\"查看\">";
    return btnText;
}
var getMarkId;
function evaluation(markId,studentId,gradeEvaluationId) {
    ...
    $.ajax({
        ...
        success: function (rs) {
            ...
            if(rs.data[2]!=null){
                $("#checkDimension").text(rs.data[2].dimension_name);
            }else{
                $("#checkDimension").text("暂无");
            }

            if(rs.data[0].markContent!=null){
                $("#teacherComment").text(rs.data[0].markContent);
            }else {
                $("#teacherComment").text("暂无");
            }
            ...
            $("#getComment").val(rs.data[0].sysContent);
            ...
            for(var j=0;j<problemInfo.length;j++){
                var getKey=problemInfo[j].answerEntity[0].solutionIds;
                var allKey=problemInfo[j].solutionEntity;
                k=j+1;
